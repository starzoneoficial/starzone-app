...
class credentials {
    constructor(BrowserWindow, mainWindow, settings, ipcMain) {
        ...
        ipcMain.on('checkMasterRegister', (event, data) => {
            var t = this.checkMasterRegister(data);

            if (t == -1){
                this.window.webContents.send("messageToAlert", "Incorrect old password. Please check it.");
            }
            
            if (t == 1){
                this.window.webContents.send("messageToAlert", "Master password changed succesfully. Your new password is: " + data[1]);
                this.createWindow("list");
            }
        ...
        ipcMain.on('userChangeMasterPassword', (event, arg) => {
            this.createWindow("userChangeMasterPassword");
        });
    ...
    createWindow(type) {
        ...
        switch (type) {
            ...
            case "userRegister":
                ...
                break;
            case "userChangeMasterPassword":
                this.window.setSize(380, 400);
                this.window.loadFile("./credentials/html/userChangeMasterPassword.html");
    ...
    checkMasterRegister(data) {

        let backup = this.settings.getSync();

        let hashMaster = crypt.hash(data[0]);

        if (crypt.decrypt(backup.master) != hashMaster) {
            return -1;
        }

        this.hashMaster = crypt.hash(data[1]);
        ...
        if ("accounts" in backup) {
            backup.accounts = JSON.parse(crypt.decrypt({ "iv": hashMaster, "content": backup.accounts }));
        } else {
            backup.accounts = [];
        }
